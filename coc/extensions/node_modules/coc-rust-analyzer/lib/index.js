...
const ctx_1 = __webpack_require__(35);
const downloader_1 = __webpack_require__(43);
const inlay_hints_1 = __webpack_require__(69);
async function activate(context) {
    const config = new config_1.Config();
    const ctx = new ctx_1.Ctx(context, config);
    const serverRoot = context.storagePath;
    if (!fs_1.existsSync(serverRoot)) {
        fs_1.mkdirSync(serverRoot);
    }
    const bin = ctx.resolveBin();
    if (!bin) {
        let msg = 'Rust Analyzer is not found, download from GitHub release?';
        const ret = await coc_nvim_1.workspace.showQuickpick(['Yes', 'Cancel'], msg);
        if (ret === 0) {
            try {
                await downloader_1.downloadServer(context, config.channel);
            }
            catch (e) {
                console.error(e);
    ...
    ctx.registerCommand('reload', (ctx) => {
        return async () => {
            coc_nvim_1.workspace.showMessage(`Reloading rust-analyzer...`);
            ...
            coc_nvim_1.workspace.showMessage(`Reloaded rust-analyzer`);
        };
    });
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.resolveCodeAction = exports.applySnippetWorkspaceEditCommand = exports.applySnippetWorkspaceEdit = exports.toggleInlayHints = exports.upgrade = exports.showReferences = exports.reloadWorkspace = exports.expandMacro = exports.syntaxTree = exports.runSingle = exports.run = exports.serverVersion = exports.ssr = exports.parentModule = exports.joinLines = exports.matchingBrace = exports.memoryUsage = exports.analyzerStatus = void 0;
const child_process_1 = __webpack_require__(4);
const coc_nvim_1 = __webpack_require__(1);
const vscode_languageserver_protocol_1 = __webpack_require__(5);
const ctx_1 = __webpack_require__(35);
const ra = __importStar(__webpack_require__(67));
...
function analyzerStatus(ctx) {
    return async () => {
        ...
        if (!ctx_1.isRustDocument(document))
            return;
        const params = {
            textDocument: { uri: document.uri },
        };
        const ret = await ctx.client.sendRequest(ra.analyzerStatus, params);
        coc_nvim_1.workspace.echoLines(ret.split('\n'));
    };
}
...
function matchingBrace(ctx) {
    return async () => {
        ...
        if (response.length > 0) {
            coc_nvim_1.workspace.jumpTo(document.uri, response[0]);
        }
    };
}
...
function joinLines(ctx) {
    return async () => {
        ...
        if (!ctx_1.isRustDocument(doc.textDocument))
            return;
...
function parentModule(ctx) {
    return async () => {
        ...
        if (response.length > 0) {
            ...
            coc_nvim_1.workspace.jumpTo(uri, range.start);
        }
    };
}
...
function ssr(ctx) {
    return async () => {
        const input = await coc_nvim_1.workspace.callAsync('input', ['Enter request like this: foo($a:expr, $b:expr) ==>> bar($a, foo($b)): ']);
        coc_nvim_1.workspace.nvim.command('normal! :<C-u>', true);
        if (!input) {
            return;
        }
        if (!input.includes('==>>')) {
...
function serverVersion(ctx) {
    return async () => {
        ...
        const version = child_process_1.spawnSync(bin, ['--version'], { encoding: 'utf-8' }).stdout;
        coc_nvim_1.workspace.showMessage(version);
    };
}
...
function run(ctx) {
    return async () => {
        ...
        if (!ctx_1.isRustDocument(document))
            return;
        coc_nvim_1.workspace.showMessage(`Fetching runnable...`);
        const params = {
            textDocument: { uri: document.uri },
            position,
        };
        ...
        const idx = await coc_nvim_1.workspace.showQuickpick(items.map((o) => o.label));
        if (idx === -1) {
            return;
        }
        ...
        coc_nvim_1.workspace.createTerminal(opt).then((t) => {
            t.sendText(cmd);
        });
    };
}
...
function runSingle() {
    return async (runnable) => {
        const { document } = await coc_nvim_1.workspace.getCurrentState();
        if (!runnable || !ctx_1.isRustDocument(document))
            return;
...
function syntaxTree(ctx) {
    return async () => {
        ...
        let range = null;
        if (mode) {
            range = await coc_nvim_1.workspace.getSelectedRange(mode, doc);
        }
        const param = {
            textDocument: { uri: doc.uri },
            range,
        ...
        await coc_nvim_1.workspace.nvim.command('tabnew').then(async () => {
...
function showReferences() {
    return async (uri, position, locations) => {
        ...
        await coc_nvim_1.commands.executeCommand('editor.action.showReferences', coc_nvim_1.Uri.parse(uri), position, locations);
    };
}
...
function toggleInlayHints(ctx) {
    return async () => {
        if (!ctx.config.inlayHints.chainingHints) {
            coc_nvim_1.workspace.showMessage(`Inlay hints for method chains is disabled. Toggle action does nothing;`, 'warning');
            return;
        }
        for (const sub of ctx.subscriptions) {
            // @ts-ignore
            if (typeof sub.toggle === 'function')
                sub.toggle();
        }
    };
}
...
async function applySnippetWorkspaceEdit(edit) {
    var _a;
    if (!((_a = edit.documentChanges) === null || _a === void 0 ? void 0 : _a.length)) {
        return;
    }
    ...
    if (vscode_languageserver_protocol_1.TextDocumentEdit.is(change)) {
        ...
        for (const indel of change.edits) {
            ...
            if (parsed) {
                ...
                if (placeholderLength) {
                    selection = vscode_languageserver_protocol_1.Range.create(startLine, startColumn, startLine, startColumn + placeholderLength);
                }
                else {
                    position = vscode_languageserver_protocol_1.Position.create(startLine, startColumn);
                }
                newText = insert;
            }
            else {
            ...
            newEdits.push(vscode_languageserver_protocol_1.TextEdit.replace(indel.range, newText));
        }
        const wsEdit = {
            changes: {
        ...
        const current = await coc_nvim_1.workspace.document;
        if (current.uri !== change.textDocument.uri) {
            await coc_nvim_1.workspace.loadFile(change.textDocument.uri);
            await coc_nvim_1.workspace.jumpTo(change.textDocument.uri);
        }
        if (selection) {
            await coc_nvim_1.workspace.selectRange(selection);
        }
        else if (position) {
            await coc_nvim_1.workspace.moveTo(position);
        }
    }
}
...
function resolveCodeAction(ctx) {
    return async (params) => {
        const edit = await ctx.client.sendRequest(ra.resolveCodeAction, params);
        if (!edit)
            return;
        await applySnippetWorkspaceEdit(edit);
    };
...
const callHierarchy = __webpack_require__(33);
const st = __webpack_require__(34);
var Proposed;
(function (Proposed) {
...
const protocol_foldingRange_1 = __webpack_require__(29);
exports.FoldingRangeRequest = protocol_foldingRange_1.FoldingRangeRequest;
const protocol_declaration_1 = __webpack_require__(30);
exports.DeclarationRequest = protocol_declaration_1.DeclarationRequest;
const protocol_selectionRange_1 = __webpack_require__(31);
exports.SelectionRangeRequest = protocol_selectionRange_1.SelectionRangeRequest;
const protocol_progress_1 = __webpack_require__(32);
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.Ctx = exports.isRustDocument = void 0;
const coc_nvim_1 = __webpack_require__(1);
const executable_1 = __importDefault(__webpack_require__(36));
const fs_1 = __webpack_require__(2);
const os_1 = __webpack_require__(16);
const path_1 = __webpack_require__(15);
const vscode_languageserver_protocol_1 = __webpack_require__(5);
const which_1 = __importDefault(__webpack_require__(38));
const client_1 = __webpack_require__(42);
const downloader_1 = __webpack_require__(43);
const ra = __importStar(__webpack_require__(67));
...
class Ctx {
    constructor(extCtx, config) {
        this.extCtx = extCtx;
        this.config = config;
        this.sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
        this.statusBar = coc_nvim_1.workspace.createStatusBarItem(10);
    ...
    registerCommand(name, factory) {
        ...
        const d = coc_nvim_1.commands.registerCommand(fullName, cmd);
    ...
    async startServer() {
        ...
        const client = client_1.createClient(bin);
        this.extCtx.subscriptions.push(coc_nvim_1.services.registLanguageClient(client));
        ...
        client.onNotification(ra.status, async (params) => {
            ...
            if (status === 'ready') {
                ...
                const prompt = this.config.cargo.autoreload || (await coc_nvim_1.workspace.showPrompt(`rust-analyzer needs to reload project`));
                if (prompt) {
                    await coc_nvim_1.commands.executeCommand('rust-analyzer.reloadWorkspace');
                    this.statusBar.hide();
                }
            }
        });
    ...
    resolveBin() {
        ...
        // 2. bundled
        let bin = path_1.join(this.extCtx.storagePath, process.platform === 'win32' ? 'rust-analyzer.exe' : 'rust-analyzer');
        if (this.config.serverPath) {
            bin = this.config.serverPath;
            if (bin.startsWith('~/')) {
                bin = bin.replace('~', os_1.homedir());
            }
            bin = which_1.default.sync(bin, { nothrow: true }) || bin;
        }
        if (!fs_1.existsSync(bin)) {
            return;
        }
        if (!executable_1.default.sync(bin)) {
            coc_nvim_1.workspace.showMessage(`${bin} is not executable`, 'error');
            return;
        }
        return bin;
    }
    async checkUpdate(auto = true) {
        ...
        const latest = await downloader_1.getLatestRelease(this.config.channel);
        if (!latest) {
            return;
        }
        ...
        if (old === latest.tag) {
            if (!auto) {
                coc_nvim_1.workspace.showMessage(`Your Rust Analyzer release is updated`);
            }
            return;
        }
        const msg = `Rust Analyzer has a new release: ${latest.tag}, you're using ${old}. Would you like to download from GitHub`;
        const ret = await coc_nvim_1.workspace.showQuickpick(['Yes, download the latest rust-analyzer', 'Check GitHub releases', 'Cancel'], msg);
        if (ret === 0) {
            try {
                await downloader_1.downloadServer(this.extCtx, this.config.channel);
            }
            catch (e) {
                console.error(e);
            ...
            await coc_nvim_1.commands.executeCommand('vscode.open', 'https://github.com/rust-analyzer/rust-analyzer/releases').catch(() => { });
        }
    }
    async sendRequestWithRetry(reqType, param, token) {
        for (const delay of [2, 4, 6, 8, 10, null]) {
            try {
                ...
                if (error.code !== vscode_languageserver_protocol_1.ErrorCodes.ContentModified) {
                    throw error;
                }
...
const pify = __webpack_require__(37);
...
const isexe = __webpack_require__(39)
...
function createClient(bin) {
    var _a;
    let folder = '.';
    if (((_a = coc_nvim_1.workspace.workspaceFolder) === null || _a === void 0 ? void 0 : _a.uri.length) > 0) {
        folder = coc_nvim_1.Uri.parse(coc_nvim_1.workspace.workspaceFolder.uri).fsPath;
    }
    const run = {
        command: bin,
        options: { cwd: folder },
    ...
    const outputChannel = coc_nvim_1.workspace.createOutputChannel('Rust Analyzer Language Server Trace');
    const clientOptions = {
        documentSelector: [{ language: 'rust' }, { pattern: 'Cargo.toml' }],
        initializationOptions: coc_nvim_1.workspace.getConfiguration('rust-analyzer'),
        middleware: {
            async provideCodeActions(document, range, context, token) {
                ...
                const values = await client.sendRequest(vscode_languageserver_protocol_1.CodeActionRequest.type, params, token);
                if (values === null)
                    return undefined;
                const result = [];
                for (const item of values) {
                    // In our case we expect to get code edits only from diagnostics
                    if (vscode_languageserver_protocol_1.CodeAction.is(item)) {
                        result.push(item);
                        continue;
                    }
                    ...
                    result.push(vscode_languageserver_protocol_1.CodeAction.create(item.title, command));
                }
                return result;
            },
        },
    ...
    const client = new coc_nvim_1.LanguageClient('rust-analyzer', 'Rust Analyzer Language Server', serverOptions, clientOptions);
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.downloadServer = exports.getLatestRelease = void 0;
const child_process_1 = __webpack_require__(4);
const coc_nvim_1 = __webpack_require__(1);
const crypto_1 = __webpack_require__(17);
const fs_1 = __webpack_require__(2);
const https_proxy_agent_1 = __webpack_require__(44);
const node_fetch_1 = __importDefault(__webpack_require__(62));
const os_1 = __importDefault(__webpack_require__(16));
const path_1 = __importDefault(__webpack_require__(15));
const stream_1 = __importDefault(__webpack_require__(63));
const util_1 = __importDefault(__webpack_require__(55));
const pipeline = util_1.default.promisify(stream_1.default.pipeline);
const agent = process.env.https_proxy ? new https_proxy_agent_1.HttpsProxyAgent(process.env.https_proxy) : null;
...
`;
    ...
    await fs_1.promises.rename(dest, origFile);
    await new Promise((resolve, reject) => {
        var _a, _b;
        const handle = child_process_1.exec(`nix-build -E - --arg src '${origFile}' -o ${dest}`, (err, stdout, stderr) => {
            if (err != null) {
                reject(Error(stderr));
            }
            else {
    ...
    await fs_1.promises.unlink(origFile);
}
async function getLatestRelease(updatesChannel) {
    const fix = { win32: '-windows', darwin: '-mac' }[os_1.default.platform()] || '-linux';
    ...
    // @ts-ignore
    return node_fetch_1.default(releaseURL, { agent })
        .then((resp) => resp.json())
        .then((resp) => {
        const asset = resp.assets.find((val) => val.browser_download_url.includes(fix));
        const name = asset.name.replace(fix, '');
...
async function downloadServer(context, updatesChannel) {
    ...
    if (!latest) {
        statusItem.hide();
        coc_nvim_1.workspace.showMessage(`Can't get latest rust-analyzer release`);
        return;
    }
    statusItem.text = `Downloading rust-analyzer ${latest.tag}`;
    // @ts-ignore
    const resp = await node_fetch_1.default(latest.url, { agent });
    ...
    const _path = path_1.default.join(context.storagePath, latest.name);
    const randomHex = crypto_1.randomBytes(5).toString('hex');
    const tempFile = path_1.default.join(context.storagePath, `${latest.name}${randomHex}`);
    const destFileStream = fs_1.createWriteStream(tempFile, { mode: 0o755 });
    ...
    await fs_1.promises.unlink(_path).catch((err) => {
        if (err.code !== 'ENOENT')
            throw err;
    });
    await fs_1.promises.rename(tempFile, _path);
    await context.globalState.update('release', latest.tag);
    try {
        if (await fs_1.promises.stat('/etc/nixos')) {
...
const agent_1 = __importDefault(__webpack_require__(45));
...
Object.defineProperty(exports, "__esModule", { value: true });
const net_1 = __importDefault(__webpack_require__(18));
const tls_1 = __importDefault(__webpack_require__(46));
const url_1 = __importDefault(__webpack_require__(47));
const assert_1 = __importDefault(__webpack_require__(48));
const debug_1 = __importDefault(__webpack_require__(49));
const agent_base_1 = __webpack_require__(58);
const parse_proxy_response_1 = __importDefault(__webpack_require__(61));
...
const tty = __webpack_require__(54);
...
try {
	...
	const supportsColor = __webpack_require__(56);
...
/* harmony import */ var stream__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(63);
/* harmony import */ var http__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(64);
/* harmony import */ var url__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(47);
/* harmony import */ var https__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(65);
/* harmony import */ var zlib__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(66);
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.ssr = exports.inlayHints = exports.runnables = exports.onEnter = exports.joinLines = exports.resolveCodeAction = exports.parentModule = exports.matchingBrace = exports.expandMacro = exports.syntaxTree = exports.reloadWorkspace = exports.status = exports.memoryUsage = exports.analyzerStatus = void 0;
const lc = __importStar(__webpack_require__(5));
exports.analyzerStatus = new lc.RequestType('rust-analyzer/analyzerStatus');
exports.memoryUsage = new lc.RequestType0('rust-analyzer/memoryUsage');
exports.status = new lc.NotificationType('rust-analyzer/status');
exports.reloadWorkspace = new lc.RequestType0('rust-analyzer/reloadWorkspace');
...
class Config {
    constructor() {
        coc_nvim_1.workspace.onDidChangeConfiguration((event) => this.onConfigChange(event));
        this.cfg = coc_nvim_1.workspace.getConfiguration(Config.rootSection);
    }
    async onConfigChange(event) {
        ...
        const prompt = await coc_nvim_1.workspace.showPrompt(`${msg}. Reload now?`);
        if (prompt) {
            await coc_nvim_1.commands.executeCommand(`workbench.action.reloadWindow`);
        }
    }
    get serverPath() {
    ...
    get inlayHints() {
        const hasVirtualText = coc_nvim_1.workspace.isNvim && coc_nvim_1.workspace.nvim.hasFunction('nvim_buf_set_virtual_text');
        return {
...
Config.rootSection = 'rust-analyzer';
Config.requiresReloadOpts = ['serverPath', 'cargo', 'procMacro', 'files', 'updates', 'lens', 'hoverActions'].map((opt) => `${Config.rootSection}.${opt}`);
...
class HintsUpdater {
    constructor(ctx) {
        ...
        this.disposables = [];
        this.chainingHintNS = coc_nvim_1.workspace.createNameSpace('rust-chaining-hint');
        ...
        coc_nvim_1.workspace.onDidChangeTextDocument((e) => {
            const doc = coc_nvim_1.workspace.getDocument(e.bufnr);
            if (ctx_1.isRustDocument(doc.textDocument)) {
                doc.buffer.clearNamespace(this.chainingHintNS);
                if (coc_nvim_1.workspace.insertMode && !this.ctx.config.inlayHints.refreshOnInsertMode) {
                    return;
                }
                this.syncAndRenderHints();
            }
        ...
        coc_nvim_1.workspace.onDidOpenTextDocument((e) => {
            var _a;
            if (ctx_1.isRustDocument(e)) {
                const file = (_a = this.sourceFiles.get(e.uri)) !== null && _a !== void 0 ? _a : {
                    document: e,
                    inlaysRequest: null,
                };
                this.sourceFiles.set(e.uri, file);
                const doc = coc_nvim_1.workspace.getDocument(e.uri);
                doc.buffer.clearNamespace(this.chainingHintNS);
                this.syncAndRenderHints();
            }
        }, this, this.disposables);
    ...
    async syncAndRenderHints() {
        if (!this.chainingHintEnabled)
            return;
        const current = await coc_nvim_1.workspace.document;
        this.sourceFiles.forEach((file, uri) => this.fetchHints(file).then(async (hints) => {
            if (!hints)
                return;
            if (current && current.uri === uri && ctx_1.isRustDocument(current.textDocument)) {
                const decorations = this.hintsToDecorations(hints);
                this.renderDecorations(current, decorations);
            }
        }));
    }
    async renderDecorations(doc, decorations) {
        doc.buffer.clearNamespace(this.chainingHintNS);
    ...
    hintsToDecorations(hints) {
        ...
        for (const hint of hints) {
            ...
            if (hint.kind === "ChainingHint" /* ChainingHint */) {
...
function activateInlayHints(ctx) {
    const maybeUpdater = {
        ...
        async onConfigChange() {
            ...
            await ctx.sleep(100);
            await coc_nvim_1.workspace.nvim.command('hi default link CocRustChainingHint CocHintSign');
            if (this.updater) {
        ...
        toggle() {
            var _a;
            (_a = this.updater) === null || _a === void 0 ? void 0 : _a.toggle();
        },
        dispose() {
            var _a;
    ...
    coc_nvim_1.workspace.onDidChangeConfiguration(maybeUpdater.onConfigChange, maybeUpdater, ctx.subscriptions);
