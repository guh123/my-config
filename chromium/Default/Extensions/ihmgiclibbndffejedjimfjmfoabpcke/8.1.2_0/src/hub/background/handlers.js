...
(function (undefined) {
    ...
    pl.extend(ke.app.handlers, {
        ...
        checkSponsorships: function (on_done_callback) {
                ...
                if (navigator.onLine) {
                    $.ajax({
                        ...
                        success: (f) => {
                            ...
                            // set it to max value, so the bar isn't hidden (bc Date.now() - last claim would be always Date.now())
                            // and so the bar isn't hidden before the user actually clicks Claim
                            // only do this if there's no previous value
                            if (typeof ke.ext.util.storageUtil.getJsonField('sponsorships_last_claim', f.service_id) === "undefined") {
                                ke.ext.util.storageUtil.setJsonField('sponsorships_last_claim', f.service_id, Number.MAX_SAFE_INTEGER);
                            }
                    ...
                    window.addEventListener('online', function () {
                        ke.app.handlers.checkSponsorships(on_done_callback);
                    });
                }
            }
        },
