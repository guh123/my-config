    ...
    pl.extend(ke.app.render, {
        ...
        organize: {
            ...
            tryShowSponsorshipBar: function() {
                ...
                // Not show if:
                //   - data is corrupted or missing (couldn't fetch it from the server, for ex)
                //   - the ad is expired
                //   - user's locale is important but not supported by the service's LP (as of now, it's Chinese & Russian)
                if (!validateSponsorships() 
                    || !ke.app.temp.sponsorships.active 
                    || Date.now() - ke.app.temp.last_claim_click[ke.app.temp.sponsorships.service_id] >= ke.app.temp.sponsorships.expires_in
                    || (ke.app.temp.sponsorships.important_locales.indexOf(user_lang) > -1 && !(user_lang in ke.app.temp.sponsorships.links))) {

                    ke.app.handlers.hideSponsorshipsBar();
                } else {
                    $('.sponsorship-bar')
                        .html(ke.getLocale('Sponsorship_Bar')
                            .replace('{{percentage_off}}', ke.app.temp.sponsorships.percentage_off)
                            .replace('{{service_name}}', ke.app.temp.sponsorships.service_name))
                        .on('click', ke.app.handlers.showSponsorship);
                }
            },
